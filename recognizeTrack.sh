...
#Change the character encording to UTF-8 to allow for names with special characters
export LANG=en_US.UTF-8
#locale charmap
cd /home/claire/AudioFingerprinting/dejavu
#Run the fingerprinting program and save the result in a variable
output=$(python3 dejavu.py --recognize file /home/claire/AudioFingerprinting/staging/audioclip.wav)
...
if (( confidence > 20 )); then
    ...
    if [ "${sList[0]}" = "$song" ] && [ "${sList[1]}" = "$artist" ] && (( difference < 86400 )); then
	    ...
	    input=$artist" - "$song
	    python3 pylast/scrobble.py $input
	...
	echo "Bad confidence, no scrobble"
fi
exit 0
